...
#Parameters
Wp=590 #Pump beam size(μm) 590のときd=28.5で31の効率Max
...
def Eff(W,L):
    A= quad(funcA, 0, np.inf, (Pp, Lp, Wp, Pl, L, W))[0]*quad(funcC, 0, 2*np.pi, (L))[0]
    B= (quad(funcB, 0, np.inf, (Pp, Lp, Wp))[0]*quad(funcC, 0, 2*np.pi, (L))[0] * quad(funcB, 0, np.inf, (Pl,L,W))[0]*quad(funcC, 0, 2*np.pi, (L))[0])**0.5
    eff=A/B
    return eff
...
for i in range(11):
    print('Wl=',Wl[i],'','Ll=',Ll[i],'','eff=',Eff(Wl[i],Ll[i]))
